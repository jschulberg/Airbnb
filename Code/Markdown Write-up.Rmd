...
```{r cook's distance plot1, echo=TRUE}
...
```{r cook's removals, echo=TRUE, warning=FALSE}
...
# that it has to do with amount spent in previous year, so let's break this out further.
...
```{r gender/survival relationship}
...
```{r survival/gender relationship2}
...
Thus, the probability that a female survives is 66.7% and the probability that a male survives is 16.7%. This makes sense because, at least in the movie, they tended
to put women and children on life boats. If we brought in age too, I'd expect that most of the males who survived were young (children).  

```{r visualize_gender_survival}
